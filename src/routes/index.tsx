...
export const Route = createFileRoute('/')({
  head: () => ({
    meta: [
      ...
      { name: 'description', content: 'Résolvez et visualisez pas à pas les problèmes de transport linéaire : Balas-Hammer, MODI, Stepping-Stone.' },
      { property: 'og:title', content: 'TRANSP — Solveur de transport' },
      { property: 'og:description', content: 'Méthodes Nord-Ouest, MINILI, MINICO, MINITAB, Balas-Hammer + Stepping-Stone & MODI.' },
    ],
  }),
  component: Index,
});
...
function Index() {
  const { initMethod, setInitMethod, optimMethod, setOptimMethod, solve, reset, initResult, optimResult } = useStore();
      ...
      <header className="border-b border-border bg-card">
        <div className="max-w-7xl mx-auto px-6 py-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Solveur pédagogique de transport linéaire · Balas-Hammer · MODI · Stepping-Stone</p>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        <section className="space-y-4">
          <h2 className="font-display text-xl flex items-center gap-2">
            <span className="w-7 h-7 rounded bg-accent text-accent-foreground text-sm flex items-center justify-center font-bold">1</span>
            Saisie du problème
          </h2>
          <ProblemEditor />
        </section>
        ...
        <section className="space-y-4">
          <h2 className="font-display text-xl flex items-center gap-2">
            <span className="w-7 h-7 rounded bg-accent text-accent-foreground text-sm flex items-center justify-center font-bold">2</span>
            Choix des méthodes
          </h2>
          <div className="grid md:grid-cols-2 gap-4">
            <div className="rounded-lg border border-border bg-card p-4">
              <h3 className="font-display text-sm uppercase tracking-wider text-muted-foreground mb-3">Initialisation</h3>
            ...
            <div className="rounded-lg border border-border bg-card p-4">
              <h3 className="font-display text-sm uppercase tracking-wider text-muted-foreground mb-3">Optimisation</h3>
              <div className="space-y-2">
                {OPTIM_METHODS.map(m => (
                  <button
                    key={m.id}
              ...
              <div className="mt-6 flex gap-2">
                <Button onClick={solve} size="lg" className="flex-1 bg-primary hover:bg-primary/90">
                  <Play className="w-4 h-4 mr-2" />Résoudre
                </Button>
        ...
        {initResult && optimResult && (
          <section className="space-y-4">
            <h2 className="font-display text-xl flex items-center gap-2">
              <span className="w-7 h-7 rounded bg-accent text-accent-foreground text-sm flex items-center justify-center font-bold">3</span>
              Résolution pas à pas
            </h2>
            <Tabs defaultValue="optim" className="w-full">
              <TabsList>
                <TabsTrigger value="init">Phase A · Initialisation ({initResult.steps.length})</TabsTrigger>
                <TabsTrigger value="optim">Phase B · Optimisation ({optimResult.iterations} iter.)</TabsTrigger>
                <TabsTrigger value="compare">Comparaison des méthodes</TabsTrigger>
              </TabsList>
              <TabsContent value="init" className="mt-4"><InitViewer /></TabsContent>
              <TabsContent value="optim" className="mt-4"><OptimViewer /></TabsContent>
              <TabsContent value="compare" className="mt-4"><MethodComparison /></TabsContent>
            </Tabs>
          </section>
        )}
        ...
        {optimResult && (
          <section className="space-y-4">
            <h2 className="font-display text-xl flex items-center gap-2">
              <span className="w-7 h-7 rounded bg-accent text-accent-foreground text-sm flex items-center justify-center font-bold">4</span>
              Rapport final
            </h2>
            <FinalReport />
          </section>
        ...
        <footer className="text-center text-xs text-muted-foreground pt-8 pb-4">
          Basé sur les cours RO_TRANSP_v3 (2013–2015) · Programmation linéaire — Problème de transport
        </footer>
      </main>
    </div>
  );
