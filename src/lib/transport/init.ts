...
import {
  Allocation, InitMethod, InitResult, InitStep, TransportProblem,
  cloneAllocs, computeCost, fixDegeneracy,
} from './core';
...
export function solveInit(method: InitMethod, p: TransportProblem): InitResult {
  switch (method) {
    case 'NorthWest': return northWest(p);
    case 'MINILI': return miniLi(p);
    case 'MINICO': return miniCo(p);
    case 'MINITAB': return miniTab(p);
    case 'BHammer': return bHammer(p);
  }
}

function finalize(method: InitMethod, allocations: Allocation[], steps: InitStep[], p: TransportProblem): InitResult {
...
function northWest(p: TransportProblem): InitResult {
  ...
  while (i < m && j < n) {
    ...
    steps.push(snapshot(
      ...
      'allocate', allocs, rs, rd, { highlightCell: [i, j] }
    ));
    if (rs[i] === 0 && rd[j] === 0) {
      // degenerate corner: move diagonally
      if (i < m - 1) i++; else j++;
    } else if (rs[i] === 0) i++; else j++;
  }
...
function miniLi(p: TransportProblem) { return miniByOrder(p, true, 'MINILI'); }
function miniCo(p: TransportProblem) { return miniByOrder(p, false, 'MINICO'); }
