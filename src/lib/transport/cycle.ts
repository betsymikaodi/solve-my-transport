...
export function findCycle(
  ...
  const cells: [number, number][] = [[er, ec], ...basis.filter(([i, j]) => !(i === er && j === ec))];
  ...
  while (changed) {
    ...
    const next = pool.filter(([i, j]) =>
      (i === er && j === ec) || (rowCount.get(i)! >= 2 && colCount.get(j)! >= 2)
    );
    if (next.length !== pool.length) { changed = true; pool = next; }
  }
  ...
  function dfs(axis: 'row' | 'col'): boolean {
    ...
    for (const c of candidates) {
      ...
      if (path.length >= 4) {
        ...
        if ((nextAxis === 'col' && c[1] === start[1]) || (nextAxis === 'row' && c[0] === start[0])) {
          return true;
        }
      }
...
export function pivot(
  ...
  minus.forEach(([i, j], k) => {
    ...
    if (q < theta) { theta = q; leavingIdx = k; }
  });
