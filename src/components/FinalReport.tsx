      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-sm text-muted-foreground mt-1">
            Init : <span className="font-mono">{initMethod}</span> · Optim :{" "}
            <span className="font-mono">{optimMethod}</span> · {optimResult.iterations} itération(s)
            {optimResult.multipleOptima && (
              <span className="ml-2 text-accent">· Solutions multiples</span>
            )}
            {balanced.fictiveAdded && (
              <span className="ml-2 text-accent">
                · {balanced.fictiveAdded === "col" ? "Destination" : "Origine"} fictive ajoutée
              </span>
            )}
          </div>
        </div>
        <Button onClick={exportPDF} variant="default" className="bg-primary">
          ...
          <tbody>
            {final.map((a, k) => (
              <tr key={k} className="border-t border-border">
                ...
                <td className="p-2 border border-border text-right font-mono text-muted-foreground">
                  {balanced.costs[a.row][a.col]}
                </td>
            ...
            <tr className="bg-primary text-primary-foreground">
              <td colSpan={4} className="p-3 text-right font-display text-base">
                Z TOTAL =
              </td>
              <td className="p-3 text-right font-mono font-bold text-lg">
                {optimResult.finalCost}
              </td>
            </tr>
          </tbody>
        </table>
