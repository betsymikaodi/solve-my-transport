      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-muted">
            <th className="p-2 border border-border font-display text-xs uppercase tracking-wider text-muted-foreground">cᵢⱼ</th>
            {colL.map((c, j) => (
              <th key={j} className={`p-2 border border-border font-display ${isFictiveCol && j === n - 1 ? 'text-accent italic' : ''}`}>
                {c}
              </th>
            ))}
            <th className="p-2 border border-border font-display text-xs uppercase tracking-wider text-muted-foreground">aᵢ</th>
            {p.rowPenalties && <th className="p-2 border border-border bg-secondary/10 font-display text-xs">Pén.</th>}
            {p.potentials && <th className="p-2 border border-border bg-accent/10 font-display text-xs">uᵢ</th>}
          </tr>
        </thead>
        <tbody>
          {Array.from({ length: m }).map((_, i) => (
            <tr key={i}>
              <th className={`p-2 border border-border bg-muted font-display ${isFictiveRow && i === m - 1 ? 'text-accent italic' : ''}`}>{rowL[i]}</th>
              {Array.from({ length: n }).map((_, j) => {
                const key = `${i},${j}`;
                const a = allocMap.get(key);
                const sign = cycleMap.get(key);
                const delta = deltaMap.get(key);
                const isHighlight = p.highlightCell && p.highlightCell[0] === i && p.highlightCell[1] === j;
                const isEntering = p.enteringCell && p.enteringCell[0] === i && p.enteringCell[1] === j;

                let cellCls = 'relative p-2 border border-border h-16 min-w-[64px] text-center align-middle';
                ...
                return (
                  <td key={j} className={cellCls}>
                    <div className="text-[10px] text-muted-foreground absolute top-0.5 left-1 font-mono">{problem.costs[i][j]}</div>
                    {a ? (
                      <div className={`font-display font-semibold ${a.isEpsilon ? 'text-accent italic' : 'text-foreground'}`}>
                        {a.isEpsilon ? 'ε' : a.quantity}
                      </div>
                    ) : delta !== undefined ? (
                      <div className={`text-xs font-mono ${delta < 0 ? 'text-negative font-bold' : 'text-muted-foreground'}`}>
                        Δ={delta}
                      </div>
                    ) : null}
                    {sign && (
                      <div className={`absolute bottom-0.5 right-1 text-xs font-bold ${sign === '+' ? 'text-positive' : 'text-negative'}`}>
                        {sign}
                      </div>
                    )}
                  </td>
              ...
              {p.rowPenalties && (
                <th className={`p-2 border border-border font-mono ${p.selectedAxis?.kind === 'row' && p.selectedAxis.index === i ? 'bg-destructive/20 text-destructive font-bold' : 'bg-secondary/5'}`}>
                  {p.rowPenalties[i] ?? '—'}
                </th>
              )}
              {p.potentials && (
          ...
          <tr>
            <th className="p-2 border border-border bg-muted font-display text-xs uppercase tracking-wider text-muted-foreground">bⱼ</th>
          ...
          {p.colPenalties && (
            <tr>
              ...
              {p.colPenalties.map((cp, j) => (
                <th key={j} className={`p-2 border border-border font-mono ${p.selectedAxis?.kind === 'col' && p.selectedAxis.index === j ? 'bg-destructive/20 text-destructive font-bold' : 'bg-secondary/5'}`}>
                  {cp ?? '—'}
                </th>
              ))}
          ...
          {p.potentials && (
            <tr>
              ...
              {p.potentials.v.map((vj, j) => (
                <th key={j} className="p-2 border border-border bg-accent/10 font-mono">{vj ?? '—'}</th>
              ))}
              <th className="border border-border bg-muted" />
            </tr>
          )}
