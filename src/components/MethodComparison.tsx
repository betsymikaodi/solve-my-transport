      ...
      <table className="w-full text-sm">
        <thead className="bg-muted">
          <tr>
            ...
            <th className="p-3 text-right font-display">Itérations {optimMethod === 'MODI' ? '(MODI)' : '(SS)'}</th>
        ...
        <tbody>
          {rows.map(r => (
            <tr key={r.method} className="border-t border-border">
              ...
              <td className={`p-3 text-right font-mono font-bold ${r.finalCost === best ? 'text-positive' : ''}`}>
                {r.finalCost}{r.finalCost === best && ' ★'}
              </td>
