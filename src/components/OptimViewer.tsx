  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        <Button size="sm" variant="outline" onClick={() => setOptimStep(0)}>
          <SkipBack className="w-4 h-4" />
        </Button>
        <Button
          size="sm"
          variant="outline"
          onClick={() => setOptimStep(Math.max(0, optimStepIdx - 1))}
        >
        ...
        <Button
          size="sm"
          variant="outline"
          onClick={() => setOptimStep(Math.min(optimResult.steps.length - 1, optimStepIdx + 1))}
        >
        ...
        <Button
          size="sm"
          variant="outline"
          onClick={() => setOptimStep(optimResult.steps.length - 1)}
        >
        ...
        <div className="ml-auto text-sm">
          ...
          {step.optimal && (
            <span className="ml-2 px-2 py-0.5 text-xs rounded bg-positive/20 text-positive font-bold">
              OPTIMAL ✓
            </span>
          )}
        </div>
      ...
      <div
        className={`rounded-md p-3 text-sm border ${step.optimal ? "bg-positive/10 border-positive/30" : "bg-surface border-border"}`}
      >
        {step.message}
