  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <Button size="sm" variant="outline" onClick={() => setInitStep(0)}><SkipBack className="w-4 h-4" /></Button>
        <Button size="sm" variant="outline" onClick={() => setInitStep(Math.max(0, initStepIdx - 1))}><ChevronLeft className="w-4 h-4" /></Button>
        <div className="font-mono text-sm px-3 py-1 bg-muted rounded">
          Étape {initStepIdx + 1} / {initResult.steps.length}
        </div>
        <Button size="sm" variant="outline" onClick={() => setInitStep(Math.min(initResult.steps.length - 1, initStepIdx + 1))}><ChevronRight className="w-4 h-4" /></Button>
        <Button size="sm" variant="outline" onClick={() => setInitStep(initResult.steps.length - 1)}><SkipForward className="w-4 h-4" /></Button>
