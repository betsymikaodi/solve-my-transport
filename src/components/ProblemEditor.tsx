  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-3">
        <div>
          <label className="text-xs uppercase tracking-wider text-muted-foreground font-display">Origines (m)</label>
          <Input type="number" min={2} max={8} value={m} onChange={e => setSize(+e.target.value, n)} className="w-20" />
        </div>
        <div>
          <label className="text-xs uppercase tracking-wider text-muted-foreground font-display">Destinations (n)</label>
          <Input type="number" min={2} max={8} value={n} onChange={e => setSize(m, +e.target.value)} className="w-20" />
        </div>
        <Button variant="outline" onClick={() => setProblem(EXAMPLE)}>Charger l'exemple du cours</Button>
        <div className={`ml-auto px-3 py-2 rounded-md text-sm font-mono ${balanced ? 'bg-positive/15 text-positive' : 'bg-destructive/15 text-destructive'}`}>
          {balanced ? '✓ Équilibré' : `⚠ Déséquilibre : ${totalS > totalD ? '+' : ''}${totalS - totalD}`} (Σa={totalS}, Σb={totalD})
        </div>
      </div>
        ...
        <table className="border-collapse">
          <thead>
            <tr>
              ...
              {problem.demand.map((_, j) => (
                <th key={j} className="p-1 text-xs font-display text-muted-foreground">D{j+1}</th>
              ))}
          ...
          <tbody>
            {problem.supply.map((s, i) => (
              <tr key={i}>
                <th className="p-1 text-xs font-display text-muted-foreground">{String.fromCharCode(65 + i)}</th>
                {problem.demand.map((_, j) => (
